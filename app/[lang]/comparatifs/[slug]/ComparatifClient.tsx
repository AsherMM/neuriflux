...
function ScoreBar({ value, color }: { value: number; color: string }) {
  ...
  useEffect(() => { const t = setTimeout(() => setW(value * 10), 300); return () => clearTimeout(t); }, [value]);
...
function SpiderChart({ tools, criteria }: { tools: ToolScore[]; criteria: string[] }) {
  ...
  return (
    <svg viewBox="0 0 260 260" style={{ width: "100%", maxWidth: 260 }}>
      {levels.map(level => (
        <polygon key={level} points={angles.map(a => { const p = getPoint(level * R, a); return `${p.x},${p.y}`; }).join(" ")} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
      ))}
      {angles.map((angle, i) => { const p = getPoint(R, angle); return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />; })}
      {tools.map(tool => (
        <polygon key={tool.name} points={toolPolygon(tool)} fill={`${tool.color}20`} stroke={tool.color} strokeWidth={1.5} strokeLinejoin="round" />
      ))}
      {criteria.map((label, i) => {
        const p = getPoint(R + 18, angles[i]);
        return <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,0.4)" fontSize={8} fontFamily="JetBrains Mono, monospace">{label}</text>;
      })}
    </svg>
  );
}
...
useEffect(() => {
  ...
  const L = {
    fr: {
      back: "← Comparatifs", share: copied ? "Copié !" : "Copier", winner: "🏆 Gagnant",
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .tab:hover{background:var(--bg2);color:var(--text)}.tab.active{background:var(--cyan);color:var(--bg);font-weight:600}
      ...
      <div className="layout">
        ...
        <aside className="sidebar">
          <div className="sbox">
            <div className="sbox-title">{L.scores}</div>
            {data.tools.map(tool => (
