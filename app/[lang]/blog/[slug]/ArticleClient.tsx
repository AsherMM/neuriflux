...
function renderMd(md: string): string {
  ...
  h = h.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, alt, srcValue) => {
    const cleanSrc = String(srcValue).trim();
    const cleanAlt = String(alt).trim();
    const isAllowed = cleanSrc.startsWith("/") || /^https?:\/\//i.test(cleanSrc);

    if (!isAllowed) return "";

    return `<figure class="article-md-image"><img src="${cleanSrc}" alt="${cleanAlt}" loading="lazy" decoding="async" /></figure>`;
  });
  ...
  h = h
    ...
    .map((block) => {
      ...
      if (!trimmed) return "";
      if (/^<(h[123]|ul|ol|table|pre|blockquote|p|figure)/.test(trimmed)) return trimmed;
