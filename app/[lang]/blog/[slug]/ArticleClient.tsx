...
const TAG_COLORS: Record<string, string> = {
  ...
  Productivity: "#10b981", Audio: "#ef4444", Video: "#a855f7",
  Vidéo: "#a855f7",
};
...
function renderMd(md: string): string {
  ...
  // 3. Headings (with id for TOC anchors)
  ...
  // 5. Standard markdown lists (- item)
  h = h.replace(/(^- .+\n?)+/gm, (block) => {
    const items = block.trim().split("\n").map(l => `<li>${l.replace(/^- /, "")}</li>`).join("");
    return `<ul>${items}</ul>`;
  });

  // 6. Emoji lists: lines starting with ✅ or ❌ — group consecutive ones into <ul>
  // Each ✅/❌ line becomes a <li> with the emoji preserved
  h = h.replace(/(^[✅❌].+\n?)+/gm, (block) => {
    const items = block.trim().split("\n").map(line => {
      const trimmed = line.trim();
      const emoji = trimmed.startsWith("✅") ? "✅" : "❌";
      const text = trimmed.slice(trimmed.startsWith("✅") || trimmed.startsWith("❌") ? 2 : 1).trim();
      return `<li class="emoji-${emoji === "✅" ? "yes" : "no"}"><span class="eli">${emoji}</span> ${text}</li>`;
    }).join("");
    return `<ul class="emoji-list">${items}</ul>`;
  });

  // 7. Standalone bold line (e.g. **Title:**) → treat as a subheading <p class="bold-title">
  // This handles "**Grok is not right for you if:**" as its own block
  h = h.replace(/^(<strong>[^<]+<\/strong>:?)$/gm, '<p class="bold-title">$1</p>');

  // 8. Force blank line between heading and immediately following list
  h = h.replace(/(<\/h[23]>)\n(<ul|<ol)/g, "$1\n\n$2");

  // 9. Split into blocks and wrap bare text in <p>
  h = h.split(/\n\n+/).map(block => {
    block = block.trim();
    if (!block) return "";
...
export default function ArticleClient({ lang, slug }: { lang: Lang; slug: string }) {
  ...
  const L = lang === "fr" ? {
    ...
    contact: "Contact", shareLabel: "Partager",
    ourPick: "★ Notre choix", affLink: "Lien affilié",
    ...
    contact: "Contact", shareLabel: "Share",
    ourPick: "★ Our pick", affLink: "Affiliate link",
  ...
  const schema = article && articleData ? {
    ...
    dateModified: articleData.date.en,
    url: articleUrl,
    inLanguage: lang,
  ...
  // Extract FAQ from ### headings for FAQPage schema
    ...
    <>
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{
          --bg:#080c10;--bg2:#0d1117;--bg3:#111820;--bg4:#141c24;
        ...
        .nav-links a{font-family:var(--m);font-size:.74rem;color:var(--muted);
        ...
        .art-header-glow{
          position:absolute;top:-40%;right:-10%;width:400px;height:300px;
          background:radial-gradient(ellipse,var(--art-glow,rgba(0,230,190,.07)) 0%,transparent 65%);
          pointer-events:none;
        }

        .meta{display:flex;align-items:center;gap:.65rem;flex-wrap:wrap;margin-bottom:1.25rem;position:relative;z-index:1}
        .tag-badge{font-family:var(--m);font-size:.6rem;letter-spacing:.1em;
          text-transform:uppercase;font-weight:700;padding:3px 10px;border-radius:100px}
        .art-date,.art-time{font-family:var(--m);font-size:.68rem;color:var(--muted)}
        .meta-sep{color:var(--dim);font-size:.6rem}

        .art-title{font-size:clamp(1.7rem,4vw,2.6rem);font-weight:800;letter-spacing:-.035em;
          line-height:1.1;margin-bottom:1.1rem;color:var(--text);position:relative;z-index:1}

        .art-desc{font-family:var(--m);font-size:.82rem;color:var(--muted);font-weight:300;
          line-height:1.8;padding:1rem 1.25rem;
          background:var(--bg3);border-left:2px solid var(--art-color,var(--cyan));
          border-radius:0 7px 7px 0;margin-bottom:1.75rem;position:relative;z-index:1}

        .author{display:flex;align-items:center;gap:.75rem;position:relative;z-index:1}
        .avatar{width:34px;height:34px;background:var(--cdim);
          border:1px solid rgba(0,230,190,.22);border-radius:50%;
        ...
        .prose h2{
          font-family:var(--d);font-size:1.4rem;font-weight:800;letter-spacing:-.025em;
          color:var(--text);
          margin:3rem 0 0;
          padding:.65rem 0 .65rem 1rem;
          border-left:3px solid var(--art-color,var(--cyan));
          border-bottom:1px solid var(--border);
          scroll-margin-top:80px;
        }
        .prose h2 + *{margin-top:.9rem}
        .prose h3{
          font-family:var(--d);font-size:1.05rem;font-weight:700;
          color:var(--text);margin:2rem 0 0;scroll-margin-top:80px;
        }
        .prose h3 + ul,
        .prose h3 + ol,
        .prose h3 + p{margin-top:.75rem}
        .prose p{margin-bottom:1.35rem}
        /* Drop cap uniquement sur le vrai premier paragraphe de texte, jamais sur bold-title */
        .prose p:not(.bold-title):first-of-type::first-letter{
          font-size:3.2rem;font-weight:800;font-family:var(--d);
          float:left;line-height:.8;margin:.05em .12em 0 0;
          color:var(--art-color,var(--cyan));letter-spacing:-.04em;
        }
        /* bold-title ne doit JAMAIS avoir de drop cap */
        .prose p.bold-title::first-letter{
          font-size:inherit;font-weight:inherit;float:none;
          line-height:inherit;margin:0;color:inherit;letter-spacing:inherit;
        }
        ...
        /* Emoji lists (✅ / ❌) — no bullet, items on separate lines */
        .prose ul.emoji-list{list-style:none;padding-left:0;margin-top:.75rem;margin-bottom:1.3rem}
        .prose ul.emoji-list li{display:flex;align-items:baseline;gap:.55rem;
          padding:.3rem 0;margin-bottom:0;
          border-bottom:1px solid var(--border);font-family:var(--body);font-size:1.02rem;line-height:1.65;color:#c8d5e0}
        .prose ul.emoji-list li:last-child{border-bottom:none}
        .eli{font-size:.95rem;flex-shrink:0;margin-top:.05rem}
        .emoji-yes .eli{filter:none}
        .emoji-no .eli{filter:none}

        /* Standalone bold title line */
        .prose p.bold-title{
          font-family:var(--d);font-size:1rem;font-weight:700;
          color:var(--text);letter-spacing:-.01em;
          margin-top:2rem;margin-bottom:.65rem;
          padding-left:.85rem;border-left:2px solid var(--art-color,var(--cyan));
        }
        .prose li{margin-bottom:.5rem;color:#c8d5e0;padding-left:.25rem}
        .prose li::marker{color:var(--art-color,var(--cyan));font-weight:700}
        .prose code{font-family:var(--m);font-size:.79rem;background:var(--bg3);
          border:1px solid var(--border);border-radius:4px;padding:2px 6px;color:var(--cyan)}
        .prose pre{background:var(--bg3);border:1px solid var(--border);border-radius:8px;
          padding:1.25rem 1.5rem;margin:1.75rem 0;overflow-x:auto}
        .prose pre code{background:none;border:none;padding:0;
          color:#a8c0d6;font-size:.79rem;line-height:1.78}
        .prose a{color:var(--cyan);text-decoration:none;
          border-bottom:1px solid rgba(0,230,190,.28);transition:border-color .15s}
        .prose a:hover{border-color:var(--cyan)}
        .prose blockquote{border-left:3px solid var(--art-color,var(--cyan));
          padding:.8rem 1.25rem;background:var(--bg2);
          border-radius:0 8px 8px 0;margin:1.75rem 0;
          font-style:italic;color:var(--muted);font-family:var(--body)}
        .prose table{width:100%;border-collapse:collapse;margin:1.75rem 0;
          font-family:var(--m);font-size:.75rem}
        .prose th{padding:10px 14px;border:1px solid var(--border);
          color:var(--text);font-weight:600;background:var(--bg3);text-align:left}
        .prose td{padding:10px 14px;border:1px solid var(--border);color:var(--muted)}
        .prose tr:hover td{background:var(--bg2)}

        /* ── MID CTA ─────────────────────────────────────────────── */
        .mid-cta{
          margin:2.75rem 0;padding:1.5rem 1.75rem;
          background:linear-gradient(135deg,rgba(0,230,190,.07),rgba(0,230,190,.02));
          border:1px solid rgba(0,230,190,.2);border-radius:12px;
          position:relative;overflow:hidden;
        }
        .mid-cta-stripe{position:absolute;top:0;left:0;right:0;height:2px;
          background:linear-gradient(90deg,transparent,var(--cyan),transparent)}
        .mid-cta-inner{display:flex;align-items:center;justify-content:space-between;
          gap:1.25rem;flex-wrap:wrap}
        .mid-cta-eyebrow{font-family:var(--m);font-size:.6rem;letter-spacing:.1em;
          text-transform:uppercase;color:var(--cyan);margin-bottom:.35rem}
        .mid-cta-tool{font-family:var(--d);font-size:.98rem;font-weight:700;
          color:var(--text);letter-spacing:-.02em}
        .mid-cta-label{font-family:var(--m);font-size:.68rem;color:var(--muted);
          font-weight:300;margin-top:.2rem}
        .mid-cta-btn{display:inline-flex;align-items:center;gap:.4rem;
          background:var(--cyan);color:#080c10;
          font-family:var(--d);font-weight:700;font-size:.82rem;
          padding:10px 20px;border-radius:8px;text-decoration:none;
          transition:all .18s;white-space:nowrap;flex-shrink:0}
        .mid-cta-btn:hover{opacity:.88;transform:translateY(-1px);
          box-shadow:0 4px 16px rgba(0,230,190,.28)}
        .mid-cta-note{font-family:var(--m);font-size:.52rem;color:var(--dim);
          text-align:right;margin-top:.5rem}

        /* ── END CTA ─────────────────────────────────────────────── */
        .end-cta{
          margin:3.5rem 0 0;padding:2rem 2.25rem;
          background:var(--bg2);border:1px solid rgba(0,230,190,.2);
          border-radius:16px;position:relative;overflow:hidden;
        }
        .end-cta-glow{position:absolute;top:-40%;left:50%;transform:translateX(-50%);
          width:500px;height:280px;
          background:radial-gradient(ellipse,rgba(0,230,190,.055),transparent 70%);
          pointer-events:none}
        .end-cta-stripe{position:absolute;top:0;left:50%;transform:translateX(-50%);
          width:55%;height:1px;
          background:linear-gradient(90deg,transparent,var(--cyan),transparent)}
        .end-cta-eyebrow{font-family:var(--m);font-size:.6rem;letter-spacing:.12em;
          text-transform:uppercase;color:var(--cyan);margin-bottom:.85rem;
          display:flex;align-items:center;gap:.45rem;position:relative;z-index:1}
        .end-cta-line{display:inline-block;width:16px;height:1px;background:var(--cyan)}
        .end-cta-body{display:flex;align-items:center;justify-content:space-between;
          gap:1.5rem;flex-wrap:wrap;position:relative;z-index:1}
        .end-cta-tool{font-family:var(--d);font-size:1.22rem;font-weight:800;
          letter-spacing:-.03em;color:var(--text);margin-bottom:.4rem}
        .end-cta-label{font-family:var(--m);font-size:.75rem;color:var(--muted);
          font-weight:300;line-height:1.65}
        .end-cta-actions{display:flex;flex-direction:column;gap:.4rem;align-items:flex-end}
        .end-cta-btn{display:inline-flex;align-items:center;gap:.5rem;
          background:var(--cyan);color:#080c10;
          font-family:var(--d);font-weight:700;font-size:.9rem;
          padding:12px 24px;border-radius:10px;text-decoration:none;
          white-space:nowrap;box-shadow:0 4px 20px rgba(0,230,190,.2);
          transition:all .18s}
        ...
        /* ── SHARE ───────────────────────────────────────────────── */
        .share{display:flex;align-items:center;gap:.55rem;
          margin-top:3.5rem;padding:1.25rem 1.5rem;
          background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);
          flex-wrap:wrap}
        .share-label{font-family:var(--m);font-size:.62rem;color:var(--dim);
          letter-spacing:.09em;text-transform:uppercase;margin-right:.25rem}
        .sbtn{font-family:var(--m);font-size:.7rem;padding:6px 12px;border-radius:6px;
        ...
        .sec-tag{font-family:var(--m);font-size:.62rem;letter-spacing:.14em;
        ...
        .rcard{background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);
          text-decoration:none;transition:all .18s;overflow:hidden;display:flex;flex-direction:column}
        ...
        .rcard-tag{font-family:var(--m);font-size:.57rem;letter-spacing:.09em;
          text-transform:uppercase;font-weight:600;margin-bottom:.5rem}
        .rcard-title{font-size:.85rem;font-weight:700;letter-spacing:-.01em;
          line-height:1.32;color:var(--text);margin-bottom:.6rem}
        ...
        .sbox-title{font-family:var(--m);font-size:.58rem;letter-spacing:.12em;
          text-transform:uppercase;color:var(--dim)}
        .sbox-body{padding:.85rem 1.1rem}

        /* TOC */
        .toc-list{list-style:none;display:flex;flex-direction:column;gap:0}
        .toc-item a{font-family:var(--m);font-size:.68rem;color:var(--muted);text-decoration:none;
          font-weight:300;line-height:1.45;transition:all .18s;display:block;
        ...
        /* Affiliate sidebar */
        .sbox-aff{background:linear-gradient(160deg,rgba(0,230,190,.06),rgba(0,230,190,.02));
          border-color:rgba(0,230,190,.18)!important;position:relative}
        .sbox-aff .sbox-header{border-bottom-color:rgba(0,230,190,.12)}
        .sbox-aff::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
          background:linear-gradient(90deg,transparent,var(--cyan),transparent)}
        .aff-tool{font-family:var(--d);font-size:.92rem;font-weight:800;color:var(--text);
          letter-spacing:-.02em;margin-bottom:.3rem}
        .aff-label{font-family:var(--m);font-size:.67rem;color:var(--muted);font-weight:300;
          line-height:1.55;margin-bottom:.9rem}
        .aff-btn{display:flex;align-items:center;justify-content:center;gap:.4rem;
          background:var(--cyan);color:#080c10;
          font-family:var(--d);font-weight:700;font-size:.78rem;
          padding:9px;border-radius:7px;text-decoration:none;
        ...
        /* Newsletter sidebar */
        .nl-title-s{font-family:var(--d);font-size:.85rem;font-weight:700;color:var(--text);
          letter-spacing:-.02em;margin-bottom:.3rem}
        .nl-text{font-family:var(--m);font-size:.68rem;color:var(--muted);line-height:1.65;
          font-weight:300;margin-bottom:.85rem}
        .nl-input{width:100%;background:var(--bg3);border:1px solid var(--border);
          border-radius:6px;padding:8px 11px;color:var(--text);font-family:var(--m);
          font-size:.74rem;outline:none;margin-bottom:.45rem;transition:border-color .18s}
        .nl-input:focus{border-color:rgba(0,230,190,.3)}
        .nl-input::placeholder{color:var(--dim)}
        .nl-btn{width:100%;background:var(--cyan);color:#080c10;font-family:var(--d);
          font-weight:700;font-size:.76rem;padding:9px;border-radius:6px;
          border:none;cursor:pointer;transition:opacity .18s;letter-spacing:-.01em}
        ...
        .btn-p{display:inline-flex;align-items:center;background:var(--cyan);color:#080c10;
          font-family:var(--d);font-weight:700;font-size:.86rem;padding:11px 22px;
        ...
        .ft-links a{font-family:var(--m);font-size:.62rem;color:var(--dim);
      ...
      {!article || !articleData ? (
        ...
        <div
          className="layout"
          style={{ "--art-color": color, "--art-glow": `${color}18` } as React.CSSProperties}
        >
          {/* ── MAIN ── */}
          <main>
            ...
            <div className="art-header" style={{ "--art-color": color } as React.CSSProperties}>
              ...
              <div className="meta">
                <span className="tag-badge" style={{ color, background: `${color}18`, border: `1px solid ${color}35` }}>
                  {articleData.tag}
                </span>
          ...
          <aside className="sidebar">
            ...
            {affiliate && (
              <div className="sbox sbox-aff">
                <div className="sbox-header">
                  <div className="sbox-title" style={{ color: "var(--cyan)" }}>{L.ourPick}</div>
                </div>
